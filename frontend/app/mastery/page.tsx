      ...
      <header className="border-b border-[#2a2d3e] px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[#9ca3af] text-xs mt-0.5">All documents · Learning progress</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        {avgMastery !== null && (
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-[#1c1e2e] border border-[#2a2d3e] rounded-xl p-4 text-center">
              <p className="text-3xl font-bold text-[#3ecf8e]">{avgMastery}%</p>
              <p className="text-[#9ca3af] text-xs mt-1">Avg Mastery</p>
            </div>
            <div className="bg-[#1c1e2e] border border-[#2a2d3e] rounded-xl p-4 text-center">
              <p className="text-3xl font-bold text-white">{analyzedDocs.length}</p>
              <p className="text-[#9ca3af] text-xs mt-1">Documents Studied</p>
            </div>
            <div className="bg-[#1c1e2e] border border-[#2a2d3e] rounded-xl p-4 text-center flex flex-col items-center justify-center gap-1">
              ...
              <p className="text-[#9ca3af] text-xs">Best Streak</p>
            </div>
          </div>
        )}
        ...
        {docs.length === 0 && (
          <div className="text-center py-16 space-y-3">
            <BookOpen className="w-12 h-12 text-[#9ca3af] mx-auto" />
            <p className="text-[#9ca3af]">No documents uploaded yet.</p>
            <a href="/" className="inline-block text-sm text-[#3ecf8e] hover:underline">Upload a PDF →</a>
          </div>
        )}

        {docs.length > 0 && !analyzedDocs.length && (
          <div className="text-center py-8 bg-[#1c1e2e] border border-[#2a2d3e] rounded-xl space-y-3">
            <TrendingUp className="w-10 h-10 text-[#9ca3af] mx-auto" />
            <p className="text-[#9ca3af] text-sm">No mastery data yet.</p>
            <p className="text-[#9ca3af] text-xs">Go to Chat, ask questions and take quizzes, then click &quot;Mastery&quot; → Refresh.</p>
            <a href="/" className="inline-block text-sm text-[#3ecf8e] hover:underline">Go to Chat →</a>
          </div>
        )}
        ...
        {docs.map((doc) => {
          ...
          return (
            <div key={doc.documentId} className="bg-[#1c1e2e] border border-[#2a2d3e] rounded-xl p-5 space-y-4">
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0">
                  <h3 className="text-white font-semibold text-sm truncate">{doc.filename}</h3>
                  {data && <p className="text-[#9ca3af] text-xs mt-0.5">Last analyzed {data.timestamp}</p>}
                </div>
                {data ? (
                  <span className="shrink-0 text-2xl font-bold text-[#3ecf8e]">{data.mastery.overallMastery}%</span>
                ) : (
                  <a href="/" className="shrink-0 text-xs text-[#9ca3af] hover:text-[#3ecf8e] transition-colors">Study →</a>
                )}
              </div>
                    ...
                    {data.mastery.topics.map((topic, i) => (
                      <div key={i} className="space-y-1">
                        <div className="flex items-center justify-between text-xs">
                          ...
                          <span className="text-[#9ca3af]">{topic.masteryScore}%</span>
                        </div>
              ...
              {!data && (
                <div className="border border-dashed border-[#2a2d3e] rounded-lg p-4 text-center">
                  <p className="text-[#9ca3af] text-xs">No mastery data yet. Chat about this document and take a quiz.</p>
                </div>
              )}
            </div>
          );
